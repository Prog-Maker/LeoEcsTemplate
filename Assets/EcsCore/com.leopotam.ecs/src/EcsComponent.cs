...
namespace Leopotam.Ecs
{
    ...
    [Serializable]
    public struct EcsInternalTypeDefinition
    {
        public Action<EcsEntity, object> EntityReplaceComponentBoxed;
    }
    ...
    public static class EcsComponentTypesKeeper
    {
        ...
        {
            if (EcsComponentKeepDatas == null) EcsComponentKeepDatas = new EcsComponentKeepData[0];

            var newSize = EcsComponentKeepDatas.Length + 1;
            Array.Resize(ref EcsComponentKeepDatas, newSize);
            EcsComponentKeepDatas[EcsComponentKeepDatas.Length - 1] = new EcsComponentKeepData
            {
                TypeIndex = typeIndex,
                TypeName = typeName,
                ReplaceAction = action,

                EcsInternalTypeDefinition = new EcsInternalTypeDefinition
                {
                    EntityReplaceComponentBoxed = action
                }
            };
        }

        public static Action<EcsEntity, object> GetReplaceAction(string typeName)
        {
            int length = EcsComponentKeepDatas.Length;

            for (int i = 0; i < length; i++)
            {
                var data = EcsComponentKeepDatas[i];

                if (data.TypeName == typeName)
                {
                    return data.ReplaceAction;
                }
            }
        ...
        public static EcsInternalTypeDefinition GetEcsInternalTypeDefinition(string typeName)
        {
            int length = EcsComponentKeepDatas.Length;
            ...
            for (int i = 0; i < length; i++)
            {
                ...
                if (data.TypeName == typeName)
                {
                    return data.EcsInternalTypeDefinition;
                }
            }
    ...
    public class EcsComponentKeepData
    {
        ...
        public EcsInternalTypeDefinition EcsInternalTypeDefinition;
        public Action<EcsEntity, object> ReplaceAction;
    } 

...
namespace Leopotam.Ecs2.UnityIntegration.Editor.Prototypes
{
    ...
    sealed class TemplateGenerator : ScriptableObject
    {
        ...
        const string ComponentProviderTemplate = "ComponentProvider.cs.txt";
        const string ComponentFlagTemplate = "ComponentFlag.cs.txt";
        const string SystemsProviderTemplate = "SystemsProvider.cs.txt";
        ...
        [MenuItem("Assets/Create/LeoECS/Create SystemsProvider", false, -195)]
        static void CreateSystemsProviderTpl()
        {
            CreateAndRenameAsset($"{GetAssetPath()}/EcsSystemProvider.cs", GetIcon(),
                (name) => CreateTemplateInternal(GetTemplateContent(SystemsProviderTemplate), name));
        }
        ...
        static void CreateComponentTpl()
        {
            ...
            //CreateAndRenameAsset($"{GetAssetPath()}/EcsComponent.cs", GetIcon(),
            //   (name) => CreateTemplateInternal(GetTemplateContent(ComponentProviderTemplate), name));
        }
